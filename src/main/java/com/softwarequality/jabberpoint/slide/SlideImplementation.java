...
public class SlideImplementation implements Slide {
    ...
    public List<SlideItem> getSlideItems() {
        return slideComponents.stream().filter(c -> c instanceof SlideItem).map(c -> (SlideItem) c).collect(Collectors.toList());
    }
    ...
    private float getScale(Rectangle area) {
        ...
        return Math.min(((float) area.width) / ((float) SlideConstants.WIDTH), ((float) area.height) / ((float) SlideConstants.HEIGHT));
    }
}
